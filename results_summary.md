...
1. **`IMG_076.jpg` (0.0%)** — The boxes it drew weren't close enough to the ground truth to count.
2. **`IMG_078.jpg` (0.0%)** — The boxes it drew weren't close enough to the ground truth to count.
...
2. **Tesseract draws smaller boxes:** Tesseract tries to draw tight boxes around individual words. Because these boxes are smaller, they often don't overlap enough with the ground truth to count as a "found" word. The Mean IoU metric helps show how tightly these boxes actually fit.
